...
app.put("/products/:id", (req,res) => {
    ...
    conn.query(sql, [isChecked, quantity, id] ,(err,results) => {
        if(err){return res.status(500).json({error : "internal server error"})}
        res.status(200).json({
            error : false,
            status : 200,
...
app.put("/product/note/:id",(req,res) => {
  ...
  // }
  conn.query(sql, [note,id] ,(err,results) => {
    if(err){return res.status(500).json({error : "internal server error"})}
    conn.query(getSql, [id], (err,results1) => {
      if(err){
...
app.post("/reload", async (req, res) => {
    const sqlUrl = path.join(__dirname, "./public/assets/databaseProduk.sql");
    const sqlDump = fs.readFileSync(sqlUrl, 'utf8');
    ...
    try {
      ...
      // await conn.promise().query("TRUNCATE TABLE store;"); // Ensure store table is included
...
app.delete("/products/store/:store_id", (req, res) => {
  ...
  conn.query(sql, [storeId], (err, results) => {
    if (err) {
