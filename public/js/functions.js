...
const checkAll = (check) => {
  ...
  if(check){
    ...
    axios({
      method : "put",
      url : "http://localhost:3000/allStores",
      data : {
        isChecked : 1,
        infos : info
      }
...
function toggleStoreProducts(event, store_id , checked){
  ...
  if(checked === undefined){
    ...
    if(mainCheckboxChecked){
      ...
      axios({
        method : "put",
        url : `http://localhost:3000/store/${store_id}`,
        data : {
              isChecked : isChecked,
              infos : info
            }
...
const reload = () => {
  axios.post("http://localhost:3000/reload").then(response => {
...
function deleteStore(event, storeId) {
  axios.delete(`http://localhost:3000/products/store/${storeId}`)
    .then(response => {
      if (response.status === 200) {
